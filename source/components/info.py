...
class Info():
    ...
    def get_level_display(self, level_num):
        """Chuyển đổi level number thành format hiển thị"""
        if level_num == 1:
            return '1-4'
        elif level_num == 2:
            return '2-4'
            ...
            return f'{level_num}-4'  # Fallback cho các level khác
    ...
    def create_info_labels(self):
        ...
        # Tạo stage label với format mới
        initial_level = self.get_level_display(self.game_info[c.LEVEL_NUM])
        self.create_label(self.stage_label, initial_level, 472, 55)

        # Chỉ giữ lại world_label, time_label, stage_label
        self.info_labels = [self.world_label, self.time_label, self.stage_label]
    ...
    def create_main_menu_labels(self):
        # Ẩn hết các label trong main menu
        self.state_labels = []
    ...
    def create_load_screen_labels(self):
        ...
        # Tạo stage label cho load screen với format mới
        level_display = self.get_level_display(self.game_info[c.LEVEL_NUM])
        self.create_label(self.stage_label2, level_display, 430, 200)
    ...
    def update_stage_label(self, level_num):
        """Cập nhật stage label với format mới"""
        level_display = self.get_level_display(level_num)

        # Xóa label cũ
        self.stage_label.clear()

        # Tạo label mới
        self.create_label(self.stage_label, level_display, 472, 55)

    def update_stage_label2(self, level_num):
        """Cập nhật stage label2 cho load screen với format mới"""
        level_display = self.get_level_display(level_num)

        # Xóa label cũ
        self.stage_label2.clear()
    ...
    def handle_level_state(self, level_info):
        ...
        # Cập nhật stage label với format mới
        self.update_stage_label(level_info[c.LEVEL_NUM])

        if self.state == c.LOAD_SCREEN:
            self.update_stage_label2(level_info[c.LEVEL_NUM])
